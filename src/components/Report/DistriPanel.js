...
function DistriPanel({dispatch,title,detail,distrishow}) {
    const b = ' ';
    const sheight = document.documentElement.clientHeight;
    const swidth = document.documentElement.clientWidth;
    function distriClick(){
        distrishow = !distrishow;
        dispatch({
            type:'reportinfo/distrishow',
            payload:{distrishow}
        });
    }
    return (
        <div className={st.bg_blue1+b+st.pd_5_10}>
            <div className={st.bg_white+b+st.pt_relative+b+st.bd_s+b+st.bd_w_1+b+st.bd_c_grey1+b+st.bd_r_5}>
                <img onClick={distriClick} src={distri} className={st.wd_40+b+st.hg_40+b+st.mg_5}/>
...
export default connect(mapStateToProps)(DistriPanel);
